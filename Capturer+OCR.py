...
picam2.start()
print("Initialize success, capturing")
...
def process_images(folder_path):
    ...
    x, y, w, h = 183, 155, 242, 105  # coordinate of crop
    ...
    for image_file in image_files:
        ...
        if image_path.endswith((".png", ".jpg", ".jpeg")):
            ...
            roi = gray_image[y:y + h, x:x + w]  # 裁剪灰度图的ROI
            #roi = cv2.resize(roi, None, fx=scale_factor, fy=scale_factor, interpolation=cv2.INTER_CUBIC)

            im_bw = cv2.adaptiveThreshold(
                roi, 255,
                cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                cv2.THRESH_BINARY_INV,  #  Inverse mode
                block_size,
                adaptive_c
            )
            im_bw = cv2.medianBlur(im_bw, 9)  # 去噪
            kernel = np.ones((3, 3), np.uint8)
            im_bw = cv2.dilate(im_bw, kernel, iterations=1)  # 膨胀
